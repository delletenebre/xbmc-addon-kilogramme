...
def ts_search():
    ...
    if(search_val):
        ...
        items     = [{
            'label': item['title'],
            'path' : plugin.url_for('ts_seasons', category_id = item['category_id'], name = item['name'], title = item['label'] ),
...
def ts_lastadded():
    ...
    items = [{
        'label'       : item['title'],
        'path'        : plugin.url_for('ts_seasons', category_id = item['category_id'], name = item['name'], title = to_utf8(item['label']) ) if (not item['is_playable']) else item['url'],
        'icon'        : BASE_URL + 'posters/' + item['name'] + '.jpg',
        'info'        : item['info'],
...
def ts_tvshows(category_id):
    ...
    items = [{
        'label': item['title'],
        'path' : plugin.url_for('ts_seasons', category_id = category_id, name = item['name'], title = to_utf8(item['title']) ),
        'icon' : item['icon'],
...
def ts_seasons(category_id, name):
    ...
    items     = [{
        'label'      : item['title'],
        'path'       : plugin.url_for('ts_season', category_id = category_id, name = name, season = item['season'], title = title),
